<template>
  <n-drawer v-model:show="opened" placement="right" :auto-focus="false">
    <n-drawer-content title="System settings" closable class="wrapper">
      <n-divider dashed>Theme settings</n-divider>
      <n-grid>
      ...
      <n-grid class="colors-wrapper">
        <n-grid-item v-for="(item, index) of primartyColorList" :key="index" :span="4" class="color-wrapper"
          :class="{ circle: item.checked }" :style="{ backgroundColor: item.value }" @click="colorClick(item)" />
      </n-grid>
      <div style="height: 20px"></div>
      <n-divider dashed>Menu Settings</n-divider>
      <div class="setting-item-wrapper">
        <span style="width: 150px">Menu width</span>
        <n-input-number v-model:value="menuWidth" size="small" :min="200" :max="400" :step="10">
          <template #suffix>px</template>
        </n-input-number>
      </div>
      <n-divider dashed>Page switch animation</n-divider>
      <div class="setting-item-wrapper">
        <span style="width: 100px">Animation effect</span>
...
export default defineComponent({
  ...
  setup() {
    ...
    const animOptions = [
      ...
      {
        label: 'Slide left and right',
        value: 'fade',
      },
      {
        label: 'Up and down movement',
        value: 'down',
      },
      {
