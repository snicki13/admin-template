<template>
  <n-config-provider :theme-overrides="themeOverThemes">
    <n-card :bordered="false" class="vaw-tab-split-side-bar-wrapper" :content-style="{ padding: 0 }"
      style="border-radius: 0px" :class="[
      ...
      <div class="tab-split-tab-wrapper" :style="{ backgroundColor: bgColor }">
        ...
        <div style="height: calc(100% - 48px)">
          <n-scrollbar>
            <div id="tabSplitContentWrapper" class="tab-split-content-wrapper" :style="contentWrapperStyle">
              <div v-for="item of tabs" :key="item.fullPath" class="label-item-wrapper"
                :class="{ 'vaw-tab-split-item-is-active': item.checked.value }" @click="changeTab(item)">
...
export default defineComponent({
  ...
  setup() {
    ...
    const contentWrapperStyle = computed(() => {
      return `--select-text-color: ${appConfig.theme === 'light' || appConfig.sideTheme === 'white'
          ? '#fff'
          : 'var(--text-color)'
        }`
    })
