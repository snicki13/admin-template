  ...
  <n-popover :style="{ width: '200px' }" placement="left-start" trigger="click">
    <template #default>
      <div style="border-bottom: 1px solid #f5f5f5" class="flex items-center justify-between">
        <n-checkbox v-model:checked="allChecked" :indeterminate="isIndeterminate" @update:checked="onAllChange">
          select all
        </n-checkbox>
        <n-button type="text" class="text-right"> Repossess </n-button>
      </div>
      <draggable :list="innerTableProps" animation="500" tag="transition-group" item-key="key" @end="onUpdateValue">
        <template #item="{ element }">
          <div class="flex pt-2 pb-2">
            <n-checkbox v-model:checked="element.checked" :label="element.prop" @update:checked="onChange">
              {{ element.title }}
            </n-checkbox>
