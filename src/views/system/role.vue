<template>
  <div class="main-container">
    <TableBody>
      ...
      <template #default>
        <n-data-table :loading="tableLoading" :data="dataList" :columns="tableColumns" :row-key="rowKey" />
      </template>
    </TableBody>
    ...
    <ModalDialog ref="menuModalDialogRef" title="Menu permissions" contentHeight="40vh">
      <template #content>
        <n-tree :data="menuData" checkable block-line cascade :default-expanded-keys="defaultExpandedKeys"
  ...
  setup() {
    ...
    function onDeleteItem(data: any) {
      naiveDialog.warning({
        ...
        positiveText: 'delete',
        onPositiveClick: () => {
          message.success('The simulation menu is successfully deleted, the parameter is' + JSON.stringify(data))
        },
      })
    }
    function onDataFormConfirm() {
      if (dataFormRef.value?.validator()) {
        ...
        naiveDialog.success({
          ...
          positiveText: 'Sure',
          content:
            'The simulation menu is successfully added, the parameter is:' + JSON.stringify(dataFormRef.value.generatorParams()),
        })
      }
    }
