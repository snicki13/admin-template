  ...
  setup() {
    ...
    function onConfirm() {
      if (actionModel === 'add') {
        if (dataForm.value?.validator()) {
          message.success(
            'Simulation creation menu is successful, the parameters are:' + JSON.stringify(dataForm.value?.generatorParams())
          )
        }
      } else {
        if (dataForm.value?.validator()) {
          ...
          message.success('Simulating modification menu success, the parameter is:' + JSON.stringify(params))
        }
      }
    }
    function onDeleteItem(item: any) {
      naiveDialog.warning({
        ...
        positiveText: 'delete',
        onPositiveClick: () => {
          message.success('The simulation deletion is successful, the parameter is：' + JSON.stringify(item))
        },
      })
    }
    onMounted(doRefresh)
