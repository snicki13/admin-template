<template>
  <div class="main-container">
    <n-card title="Workbench" :content-style="{ padding: '10px' }" :header-style="{ padding: '10px' }">
      <n-grid :cols="4" :y-gap="15" item-responsive responsive="screen">
        <n-grid-item class="flex" span="4 s:2 m:2 l:2 xl:2 2xl:2">
          ...
          <div class="flex flex-col justify-around ml-3.5 flex-1">
            <div class="text-lg">Good morning, Andy, youth is only once, don't let yourself be bad</div>
            <div class="text-sm text-gray-500">There are light rain today, don't forget to bring an umbrella when you go
              out ~ </div>
          </div>
        </n-grid-item>
    ...
    <n-grid cols="1 s:1 m:2 l:2 xl:2 2xl:2" :y-gap="15" :x-gap="15" responsive="screen">
      <n-grid-item>
        <n-card title="project progress" :content-style="{ padding: '5px' }" :header-style="{ padding: '10px' }">
          <n-data-table :columns="columns" :data="dataSource" :pagination="false" />
        </n-card>
      </n-grid-item>
      <n-grid-item>
        <n-card title="Project dynamics" :content-style="{ padding: '5px' }" :header-style="{ padding: '10px' }">
...
export default defineComponent({
  ...
  setup() {
    ...
    const waitingItmes = [
      ...
      {
        name: 'Old Li',
        content: 'Wait until the weekend, go shopping with colleagues, buy new clothes, buy new mobile phones, buy bags, and buy all kinds of buying and buying',
        time: '04-02',
      },
      {
        name: 'Flower brother',
